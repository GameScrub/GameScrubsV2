<template>
  ...
  >
    <div class="block" aria-haspopup="true" :aria-expanded="tooltipOpen">
      <slot name="trigger">
        <svg
...
type TooltipBg = 'light' | 'dark' | 'auto';
type TooltipSize = 'lg' | 'md' | 'sm' | 'default';
...
const positionOuterClasses = (position?: TooltipPosition): string => {
  switch (position) {
    case 'right':
...
const sizeClasses = (size?: TooltipSize): string => {
  switch (size) {
    case 'lg':
...
const colorClasses = (bg?: TooltipBg): string => {
  switch (bg) {
    case 'light':
...
const positionInnerClasses = (position?: TooltipPosition): string => {
  switch (position) {
    case 'right':
      return 'ml-2';
    case 'left':
